...
function consumer (amqp, queue, exchange, bindKey, options) {
  let ch
  let info = {}
  return Promise.try(function () {
    ...
    if (!info.exchange || !bindKey) { return }
    return ch.bindQueue(info.queue, info.exchange, bindKey).then(function () {
      info.bindKey = bindKey
    })
  }).then(function () {
